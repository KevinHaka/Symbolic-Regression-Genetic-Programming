...
class SHAP(BaseFeatureSelector):
    """
    Selects features based on SHAP values from pre-trained GP models.
    """
    def __init__(
        self,
        ...
        """
        super().__init__(n_top_features=n_top_features)

    @staticmethod
    def _lambda_func_shap(X: np.ndarray, lambda_func: Callable) -> np.ndarray:
    ...
    def _get_shap_values(
        ...
        if len(expr_variables) >= 1:
            ...
            explainer = SamplingExplainer(
                lambda X: self._lambda_func_shap(X, lambda_func),
                X_background
            )
    ...
    def select_features(
        ...
        if (self.n_top_features is None) and (n_top_features is None):
            n_features = len(X_trains[0].columns)
            n_top_features = max(1, round(np.log2(n_features)))

        elif n_top_features is None:
        ...
        selected_features, mean_shap_values_selected_features = self.select_from_pretrained_models(
    ...
    def select_from_pretrained_models(
        ...
        for gp_equation, X_train in zip(gp_equations, X_trains):
            str_variables, feature_shap_values = self._get_shap_values(X_train, gp_equation)
